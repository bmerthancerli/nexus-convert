...
export default function VideoSikistirPage() {
  ...
  const processFile = useCallback(
    async (file: File, ctx: { fetchFile: (f: File) => Promise<Uint8Array>; writeFile: (p: string, d: Uint8Array) => Promise<boolean>; exec: (a: string[]) => Promise<number>; readFile: (p: string) => Promise<Uint8Array | string>; deleteFile: (p: string) => Promise<boolean> }) => {
      ...
      await ctx.exec(["-i", inp, "-c:v", "libx264", "-crf", "28", "-preset", "medium", "-c:a", "aac", "-b:a", "128k", "output.mp4"]);
      ...
      return { blob: new Blob([bytes], { type: "video/mp4" }), filename: `${base}_compressed.mp4` };
    },
    []
  );
...
title="Video Compress"
  subtitle="Reduce video file size while preserving quality."
      headerLabel="Video Compressor"
