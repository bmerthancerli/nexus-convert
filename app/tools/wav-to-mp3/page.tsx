...
function isWavFile(file: File): boolean {
  ...
  return name.endsWith(".wav") || name.endsWith(".wave") || type === "audio/wav" || type === "audio/x-wav";
}
...
export default function WavToMp3Page() {
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      if (droppedFile && isWavFile(droppedFile)) {
        setFile(droppedFile);
        setState("ready");
        setError(null);
        setMp3Blob(null);
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      await ffmpeg.exec(["-i", inputName, "-codec:a", "libmp3lame", "-qscale:a", "2", outputName]);
    ...
    <div className="relative min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      ...
      <header className="relative border-b border-slate-200 bg-white/80 px-4 py-3 backdrop-blur-sm dark:border-slate-800 dark:bg-slate-900/80 sm:px-6">
        <div className="mx-auto flex max-w-4xl items-center justify-between">
          ...
          <span className="text-sm font-medium text-slate-500 dark:text-slate-400">WAV → MP3</span>
        </div>
      </header>

      {/* Main content - full area accepts drops when idle/ready */}
