...
export default function GorsellerdenGifPage() {
  ...
  const addFiles = useCallback((fileList: FileList | null) => {
    ...
    for (let i = 0; i < fileList.length; i++) {
      const f = fileList[i];
      if (checkFileSize(f, 20)) continue;
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      const d = Math.max(50, parseInt(delay, 10) || 100);
      ...
      for (let i = 0; i < files.length; i++) {
        const ext = files[i].name.split(".").pop() || "png";
        args.push("-loop", "1", "-t", String(duration), "-i", `img_${String(i).padStart(4, "0")}.${ext}`);
      }
      const filterInputs = files.map((_, i) => `[${i}:v]`).join("");
      args.push("-filter_complex", `${filterInputs}concat=n=${files.length}:v=1:a=0[outv]`, "-map", "[outv]", "-c:v", "gif", "-loop", "0", "output.gif");
      ...
      setOutputBlob(new Blob([bytes], { type: "image/gif" }));
      setState("done");
    } catch (err) {
      setIsLoadingFFmpeg(false);
      ...
      <main {...dropZoneProps} className="relative px-4 py-12 sm:px-6 sm:py-16">
        <div className="mx-auto max-w-2xl">
          ...
          >
            ...
            {state === "ready" && (
              <>
                <div className="w-full space-y-2 px-4 py-2 max-h-40 overflow-y-auto">
                  {files.map((f, i) => (
                ...
                <div className="mt-2">
                  <label className="text-xs text-slate-500">Kare gecikmesi (ms): </label>
                  <input type="number" value={delay} onChange={(e) => setDelay(e.target.value)} min={50} max={2000} step={50} onClick={(e) => e.stopPropagation()} className="w-20 rounded border border-slate-300 px-2 py-1 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-white" />
                </div>
                <button
