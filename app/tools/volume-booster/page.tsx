...
const AUDIO_TYPES = ["audio/mpeg", "audio/wav", "audio/x-wav", "audio/mp4", "audio/ogg", "audio/flac"];

function isAudioFile(file: File): boolean {
  const n = file.name.toLowerCase();
  const t = file.type.toLowerCase();
  return AUDIO_EXT.some((e) => n.endsWith(e)) || AUDIO_TYPES.includes(t);
}
...
export default function VolumeBoosterPage() {
  ...
  const processFile = useCallback(
    async (file: File, ctx: { fetchFile: (f: File) => Promise<Uint8Array>; writeFile: (p: string, d: Uint8Array) => Promise<boolean>; exec: (a: string[]) => Promise<number>; readFile: (p: string) => Promise<Uint8Array | string>; deleteFile: (p: string) => Promise<boolean> }) => {
      ...
      await ctx.exec(["-i", inp, "-filter:a", "volume=2.0", "output.mp3"]);
      ...
      return { blob: new Blob([bytes], { type: "audio/mpeg" }), filename: `${base}_boosted.mp3` };
    },
    []
  );
