...
export default function VideoBirlestiriciPage() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (ext !== "mp4") {
          await ffmpeg.exec(["-i", `input_${i}.${ext}`, "-c", "copy", `input_${i}.mp4`]);
          await ffmpeg.deleteFile(`input_${i}.${ext}`);
        }
      }

      const listContent = files.map((_, i) => `file 'input_${i}.mp4'`).join("\n");
      ...
      for (let i = 0; i < files.length; i++) await ffmpeg.deleteFile(`input_${i}.mp4`);
    ...
    <div className="relative min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      ...
      <main {...dropZoneProps} className="relative px-4 py-12 sm:px-6 sm:py-16">
        <div className="mx-auto max-w-2xl">
          ...
          >
            <input ref={inputRef} type="file" accept=".mp4,.webm,.avi,.mov,.mkv,video/*" multiple onChange={handleFileSelect} className="hidden" />
