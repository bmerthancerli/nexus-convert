...
export default function VideoYavaslatPage() {
  ...
  const processFile = useCallback(
    async (file: File, ctx: { fetchFile: (f: File) => Promise<Uint8Array>; writeFile: (p: string, d: Uint8Array) => Promise<boolean>; exec: (a: string[]) => Promise<number>; readFile: (p: string) => Promise<Uint8Array | string>; deleteFile: (p: string) => Promise<boolean> }) => {
      ...
      await ctx.exec(["-i", inp, "-filter:v", "setpts=2*PTS", "-filter:a", "atempo=0.5", "output.mp4"]);
      ...
      return { blob: new Blob([bytes], { type: "video/mp4" }), filename: `${base}_slow.mp4` };
    },
    []
  );
  ...
  return (
    <ToolPageLayout
      title="Video Yavaşlat"
      subtitle="Videoyu 0.5x hızda (yavaş çekim) oynatın."
      headerLabel="Video Slow Motion"
